...
const UserInfo:React.FC<UserInfoProps> = ({userUid = '', isMain = false}) => {
  const { currentUser } = useContext(AuthContext);
  const [isLinkCopied, setLinkCopied] = useState<boolean>(false)

  const copyLink = useCallback(() => {
    const link = window.location.origin + `/users?uid=${currentUser!.uid}`
  ...
  return (
    <main className={["user-info", isMain ? 'main' : ''].join(' ')}>
      {currentUser && <div className="user-info__content">
        <div className="user-info__content-left">
          <div className="user-info__info">
            <img src={currentUser.photoURL!}/>
            <h2>{currentUser.displayName}</h2>
            <div className='user-info__last-seen'>last seen 3 minutes ago</div>
            {!isMain &&    
        ...
        <div className="user-info__content-right">
          <div className="user-info__content-about about">
            ...
            <div className='about__text'>
              ...
              взял одно из самых странных слов в Lorem Ipsum, "consectetur", 
          ...
          {isMain && <div className="user-info__main-info">
              If you don't have contacts you can go and <Link to="/users">search for users to chat with</Link>.
              <br/>
